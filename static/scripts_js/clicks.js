            ...
            fetch(url, options)
                ...
                .then(data => {
                    ...
                    if (searchData.found_news % 100 >= 5 && searchData.found_news % 100 <= 20) {
                        resultContainer.innerHTML = `<h1 class="tags_finder">Найдено ${searchData.found_news} новостей с тегом "${searchData.search_query}"</h1>`;
                    } else if (searchData.found_news % 10 == 2 || searchData.found_news % 10 == 3 || searchData.found_news % 10 == 4) {
                        resultContainer.innerHTML = `<h1 class="tags_finder">Найдено ${searchData.found_news} новости с тегом "${searchData.search_query}"</h1>`;
                    } else if (searchData.found_news % 10 == 1) {
                        resultContainer.innerHTML = `<h1 class="tags_finder">Найдена ${searchData.found_news} новость с тегом "${searchData.search_query}"</h1>`;
                    } else {
                        resultContainer.innerHTML = `<h1 class="tags_finder">Найдено ${searchData.found_news} новостей с тегом "${searchData.search_query}"</h1>`;
                    }
